...
from transformers.models.bart.modeling_bart import BartDecoder, BartDecoderLayer, BartAttention, _expand_mask
from transformers.modeling_outputs import Seq2SeqLMOutput,  BaseModelOutputWithPastAndCrossAttentions, Seq2SeqModelOutput, BaseModelOutput
class SAGCopyBartForConditionalGeneration(BartForConditionalGeneration):
    
    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.model = SAGCopyBartModel(config)
    ...
    def forward(
        ...
        cross_attentions_aggregate = last_cross_attentions.mean(dim=1) #(batch_size, decoding_seq_length, encoding_seq_length)
...
class SAGCopyBartModel(BartModel):
    def __init__(self, config: BartConfig):
        super().__init__(config)
        self.decoder = SAGCopyBartDecoder(config, self.shared)
...
class SAGCopyBartDecoder(BartDecoder):
    """
    ...
    def __init__(self, config: BartConfig, embed_tokens: Optional[nn.Embedding] = None):
        super().__init__(config)
        self.layers = nn.ModuleList([BartDecoderLayer(config) for _ in range(config.decoder_layers-1)] + [SAGCopyBARTDecoderLayer(config)])
...
class SAGCopyBARTDecoderLayer(BartDecoderLayer):
    def __init__(self, config: BartConfig):
        super().__init__(config)

        self.encoder_attn = SAGCopyBartAttention(
            self.embed_dim,
...
class SAGCopyBartAttention(BartAttention):
    ...
    def __init__(
        ...
        self.w_p = torch.nn.Linear(1, embed_dim, bias=False)

    def compute_centrality_representations(self, encoder_attentions, iterations=0):
        '''
        encoder_attentions: (batch_size, num_heads, seq_len, seq_len)
        '''
        
        encoder_attentions = encoder_attentions[-1].mean(1) #(batch, seq_len, seq_len)
        T = encoder_attentions / encoder_attentions[-1].sum(1).unsqueeze(1) #(batch, seq_len, seq_len)
        scores = T.sum(1) #(batch, seq_len)

        # do TextRank update
        for _ in range(iterations):
            scores = torch.bmm(T, scores.unsqueeze(2)).squeeze(2)
        scores = scores.unsqueeze(2)
        representations = self.w_p(scores)
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_states = self._shape(self.k_proj(key_value_states) + self.compute_centrality_representations(encoder_attentions, iterations=3), -1, bsz)
